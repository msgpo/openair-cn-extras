			     ...
			     struct gtp_pktinfo *pktinfo)
{
	...
	if (gtp->role == GTP_ROLE_SGSN)
		pctx = ipv4_ebi_pdp_find(gtp, iph->saddr, (u8)(skb->mark));
	else
			...
			struct genl_info *info)
{
	...
	case GTP_V1:
		netdev_dbg(dev, "GTPv1-U: new PDP ctx id=%x/%x ssgn=%pI4 ms=%pI4 ebi %u (pdp=%p)\n",
			   ...
			   &pctx->peer_addr_ip4, &pctx->ms_addr_ip4, ebi, pctx);
		break;
	}
					    ...
					    struct nlattr *nla[])
{
	...
	if (nla[GTPA_MS_ADDRESS]) {
	  ...
	  if (nla[GTPA_BEARER_ID]) {
	    u8 ebi = nla_get_u8(nla[GTPA_BEARER_ID]);
	    return ipv4_ebi_pdp_find(gtp, ip, ebi);
	  }
...
static int gtp_genl_del_pdp(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (pctx->gtp_version == GTP_V0)
		...
		netdev_dbg(pctx->dev, "GTPv1-U: deleting tunnel id = %x/%x ebi %u (pdp %p)\n",
...
static int __init gtp_init(void)
{
	...
	pr_info("GTP module loaded (pdp ctx size %zd bytes) modified for EPS dedicated bearer/no aggregation (Proof of Concept) EURECOM www.eurecom.fr\n",
